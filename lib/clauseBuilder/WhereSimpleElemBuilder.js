/**
	The public interface of the builder which interprets and converts the simple element of a WHERE CLAUSE
	Examples of this "conds":
		{"carClass":"fullsize"}                    ==>    carClass='fullsize'
		{"price":{"lt":10}}                        ==>    price<10
		{"email":{"like":"%@yahoo.com"}}           ==>    email LIKE '%@yahoo.com'
		{"price":{"between":["0","7"]}}            ==>    price BETWEEN 0 AND 7
...
WhereSimpleElementBuilder.prototype.buildClause = function(){
	...
	//self.EClog(spaces + spaces + spaces + spaces + "WhereSimpleElementBuilder buildClause(): " + JSON.stringify([conds,allkeys]));
	if(allkeys.length == 0){
	...
	if(typeof conds[firstKey] === 'object'){  // complex clause
...
var readkeyValuePair = function(keyname,val,self){
	var finalClause = keyname + "="; //would contain the whole clause that should be returned to the caller side
...
/*
	for the clause like "gt","lt","between","like" only

	keyname:  price,conds  : {"lt":10}                    ==>    price<10
	keyname:  email,conds  : {"like":"%@yahoo.com"}       ==>    email LIKE '%@yahoo.com'
	keyname:  price,conds  : {"between":["0","7"]}        ==>    price BETWEEN 0 AND 7
