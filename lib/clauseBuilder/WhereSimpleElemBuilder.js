...
WhereSimpleElementBuilder.prototype.buildClause = function(){
	...
	debug(spaces + "        buildClause: allkeys  is: " +JSON.stringify(allkeys) + ",  the firstKey is:" + firstKey);

	if(typeof conds[firstKey] === 'object'){
		debug(spaces + "        buildClause: will do readObjectElement() for the condition: " +JSON.stringify(conds[firstKey]) );
		return readObjectElement(firstKey,conds[firstKey],self);
	}else{  
		debug(spaces + "        buildClause: will do readkeyValuePair()" );
...
var readkeyValuePair = function(keyname,val,self){
	...
	finalClause = concatValuePart(finalClause,val);
		debug(spaces + "            readkeyValuePair: clause:" + finalClause);
	return finalClause;
};
