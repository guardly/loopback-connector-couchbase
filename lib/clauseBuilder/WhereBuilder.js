...
WhereBuilder.prototype.buildClause = function(){
	...
	Object.keys(conds).forEach(function (key) {
		...
		if (self.stringEqualNoCase(key,constants.KEYWORD_LEVEL2_AND) || self.stringEqualNoCase(key,constants.KEYWORD_LEVEL2_OR))
		{
			...
			finalClause = readKeyValuePair(conds,self,constants,finalClause);
...
/*
	Reads and converts the And/Or condition
*/
	...
	for (i=0;i<conds.length;i++){
		...
		Object.keys(currentConds).forEach(function (key) {
			...
			if (self.stringEqualNoCase(key,constants.KEYWORD_LEVEL2_AND) || self.stringEqualNoCase(key,constants.KEYWORD_LEVEL2_OR)){
				...
				finalClause = readKeyValuePair(currentConds,self,constants,finalClause);
...
*/
var readKeyValuePair = function(conds,theThis,constants,finalClause){
	var self = theThis;
	finalClause = finalClause + '(';
	self.EClog(spaces + spaces + spaces + spaces + "readKeyValuePair(): " + JSON.stringify([conds,finalClause]));
