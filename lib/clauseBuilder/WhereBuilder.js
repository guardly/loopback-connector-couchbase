...
/*
	...
	--ATTENTION: the docType='model_name' should be implicitly appended to the end of where clause
	--ATTENTION: the parameter "conds" maybe null (not undefined) if there is not any where conditions specified, in this case, docType='model_name' is the only where clause
...
WhereBuilder.prototype.buildClause = function(){
	...
	if(conds){
		...
		if(model){
			finalClause = finalClause + " AND (" + constants.KEYWORD_CONVENTION_DOCTYPE + "='" + model + "')";
		};
	}else{
		if(model){
			finalClause = "WHERE (" + constants.KEYWORD_CONVENTION_DOCTYPE + "='" + model + "')";
		};
	};
