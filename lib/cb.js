...
CouchbaseDB.prototype.all = function all(model, filter, callback) {
  ...
  if (filter.hasOwnProperty('where') && filter.where.hasOwnProperty('id') && filter.hasOwnProperty('limit') && filter.limit === 1) {
 ...
 */
CouchbaseDB.prototype.destroyAll = function destroyAll(model, where, callback) {
  ...
  if (where && where.id) {
    self.destroy(model, where.id, callback);
    return;
  }
