...
CouchbaseDB.prototype.count = function (model, callback, where) {
	...
	/*
	var thisModel = this._models[model];
	var props = thisModel.properties;
	var relations = thisModel.settings.relations;

	EClog("CouchbaseDB.prototype.count() : " + JSON.stringify([model, where])); 
	EClog("CouchbaseDB.prototype.count() thisModel: " + JSON.stringify([thisModel])); 
	EClog("CouchbaseDB.prototype.count() thisModel  : " + thisModel); 
	EClog("CouchbaseDB.prototype.count() props: " + JSON.stringify([props])); 
	EClog("CouchbaseDB.prototype.count() relations: " + JSON.stringify([relations])); 
	*/
 ...
 * Will be called when  PUT to /model_url_name
 *
 * for example: PUT   /Guests
 */
...
CouchbaseDB.prototype.all = function all(model, filter, callback) {
	...
	//EClog("CouchbaseDB.prototype.all() : " + typeof model); 
 ...
 *
 * for example: PUT   /Guests/aaaa

 * strange thing: the "id" passed in does not depend on the "id" input box on the explorer, it is acutally the value of the field "id" in the data object
 */
