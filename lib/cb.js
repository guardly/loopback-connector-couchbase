...
CouchbaseDB.prototype.destroyAll = function destroyAll(model, filter,callback) {
	...
	if(isRequestedByIDOnly(filter)){
		debug("CouchbaseDB.prototype.destroyAll() : remove this doc by id : " + filter[constants.KEYWORD_CONVENTION_DOCID]); 
		this.destroy(model, filter[constants.KEYWORD_CONVENTION_DOCID], callback);
		return;
	};
	var self = this;
	var clauseBuilderIndex = new clauseBuilderClass(); 
	filter = specifyDocIdField(filter);
	var clausebuilderObj = clauseBuilderIndex.getClauseBuilder(filter,self.settings.bucket,model);
	var clause = clausebuilderObj.buildClause();
	qryString = clause;
	...
	cbquery(qryString,self,function(err,res){
		...
		async.each(res, function( eachId, cb_async) {
			...
			callback(err,{"removed":"removed " + doneRemovals.length + "records"});
		});

		/*
		for(var i=0;i<res.length;i++){
			var currentDocid = res[i][constants.KEYWORD_CONVENTION_DOCID]
			debug("    CouchbaseDB.prototype.destroyAll()   removing the document : " + currentDocid); 
			...
			removeOneDoc(currentDocid, self,function(err, result){
				...
				debug("    CouchbaseDB.prototype.destroyAll()  : removal result of '" + currentDocid + "' is:  "  + JSON.stringify([err, result]) );
				//callback(null,result);
	...
	//callback(null,{"successful":"method destroyAll was successfully called"});
};
