...
function CouchbaseDB(settings, dataSource) {
  ...
  this.dataSource.on('error', function(err) {
    console.error( 'Couchbase DS error ', err );
  });
...
CouchbaseDB.prototype.create = function (model, data, callback) {
  ...
  var myBucket = self.getBucket(function(err){
    ...
    delete data._meta;
    myBucket.insert(
      createDataBuilderObj.getIdKey(),
      createDataBuilderObj.getPreparedData(),
      meta,
      function(err, result) {
        //myBucket.disconnect();
        if (err) {
...
CouchbaseDB.prototype.updateOrCreate = function updateOrCreate(model, data, callback) {
  ...
  var myBucket = self.getBucket(function(err){
    ...
    updateOneDoc(self,myBucket,createDataBuilderObj.getIdKey(),model,createDataBuilderObj.getPreparedData(), function(err, res){
      if(err) {
...
CouchbaseDB.prototype.getBucket = function ( clbk ) {
  ...
  if( !self.myBucket ) {
    ...
    self.myBucket.on( 'connect', function(){
      console.log( 'Couchbase Connector Bucket Connected ' );
    }.bind(this));

    self.myBucket.on( 'error', function(err){
      console.error( 'Couchbase Connector Bucket Error ', err );
    });
  } else {
    process.nextTick(function () {
...
CouchbaseDB.prototype.checkReconnect = function(err) {
  if( err && err.code === 23 || err.code === 16 || err.code === 11 ) {
    // bucket is failing with timeouts or network failures
    if(this.myBucket && this.myBucket.disconnect) {
      this.myBucket.disconnect();
    }
    this.myBucket = null;
  }
};